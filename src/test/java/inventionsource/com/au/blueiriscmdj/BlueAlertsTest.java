...
public class BlueAlertsTest
{
    private static final Logger log = (Logger) LogManager.getLogger(BlueAlertsTest.class.getName());
    ...
    public void CreateAlertsTest() throws Exception {
        try {
            ...
            BlueAlerts alerts = new BlueAlerts(dataElement);

            assertNotNull("assertNotNull alerts ", alerts);
            assertTrue("size()", alerts.size() ==7);

            BlueAlerts.BlueAlert alert = alerts.get(0);
